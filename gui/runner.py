...
def run_analysis(params: AnalysisParams, progress=None) -> Report:
    """Run the pipeline, reusing cached alignment/coverage where inputs match."""
    def step(frac: float, msg: str) -> None:
        if progress:
            progress(frac, msg)

    step(0.10, "Aligning reads (reusing cache if unchanged)…")
    ...
    step(0.65, "Computing coverage (reusing cache if unchanged)…")
